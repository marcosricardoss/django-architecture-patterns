...
def test_get_task_update_view(client):
    ...
    assert response.content.count(
        b'<input id="save_bt" type="submit" value="Save" class="ui green button">') == 1
...
def test_task_create_view_page_when_do_click_on_save_button(browser):
    ...
    assert title_field.get_attribute(
    ...
    save_bt.click()
    assert browser.find_element_by_name('deadline_at').get_attribute(
        "validationMessage") == "Please fill out this field."
    ...
    save_bt.click()
    assert browser.find_element_by_name('deadline_at').get_attribute(
        "validationMessage") == "Please enter a valid value. The field is incomplete or has an invalid date."
    ...
    save_bt.click()
    assert finished_at.get_attribute(
        "validationMessage") == "Please enter a valid value. The field is incomplete or has an invalid date."
    ...
    assert browser.find_element_by_xpath(
        "/html/body/div[2]/div[1]/div").text == "Success!"
    assert browser.find_element_by_xpath(
        "/html/body/div[2]/div[1]/p").text == "Task created successfully!"
